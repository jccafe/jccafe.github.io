...
def generate_index():
    ...
    # 決定目錄排序：將「根目錄」排在最前面，其他目錄按字母排序
    sorted_dirs = sorted(files_by_dir.keys())
    if '根目錄' in sorted_dirs:
    ...
    <style>
        ...
        * {{
        ...
        .card::before {{
            ...
            width: 100%;
            height: 100%;
            background: radial-gradient(400px circle at var(--x, 0px) var(--y, 0px), rgba(99, 102, 241, 0.08), transparent 50%);
            opacity: 0;
